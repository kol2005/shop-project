...
public class UserService {
	 ...
	 * @author 505-15
	 * 
	 * 새로 작성딘 회원가입에서 회원가입을 처리할 method
	 * 
	 * email 인증방식으로 회원가입을 처리할 것이므로
	 * userVO를 파라메터로 받아서 enabled를 false로 처리하고
...
//	public int insert(UserDetailsVO userVO) {
	...
	public String insert_getToken(UserDetailsVO userVO) {
		...
		String email_token = UUID.randomUUID().toString()
				.split("-")[0].toUpperCase();
		/*
		 * email_token = UUID.randomUUID().toString(); String[] _t =
		 * email_token.split("-"); email_token = _t[0]; email_token =
		 * email_token.toUpperCase();
		 */
