 ...
 * 컨트롤러와 view.jsp간에 데이터를 교환하는 매개체 역할을 수행
 * 일반적으로 select되는 table의 칼럼을 포함하고
 * web브라우저에서 form tag를 사용하여 
 * 	컨트롤러로 데이터를 보낼때 form tag가 
 * 	포함하는 input tag들의 name값들을 포함한다
 * 
 * VO 클래스는 
 ...
 * 	예) b_date_time 필드변수에는 2020-02-26 14:06:00 형태의 문자열만
 * 		저장하도록 setter() 메서드에 관련 코드를 정의 할수 있다
 * 		또는 
 * 		나이를 저장하는 필드변수 같으면 나이의 범위를 0 ~ 100 까지
 * 		제한하는 등의 코드를 setter 메서드에 정의 할 수 있다
 * 		
 * 		성적처리를 수행한다 라면
 * 		총점을 참조하는 getter 메서드를 정의하고
 * 		여기에 과목의 총합계를 계산하는 코드를 추가한 후
 * 		결과값을 return 하도록 만들수 있다
 * 
 * 		캡슐화(투명성)
