 ...
 * 호출을 할때
 * context/comment/list 라고 path를 지정한다
 */
...
public class CommentController {
	...
	public String insert(CommentVO commentVO,Model model) {
		...
		//String b_id = commentVO.getC_b_id()+"";
		//return "redirect:/detail?b_id=" + b_id;
		 ...
		 * 일반적인 방법 ?변수=값&변수1=값 형식으로 문자열 연결연산을
		 * 수행해야 하는데
		 * model에 Attribute로 해당 값을 add 수행한 후
		 * redirect를 수행하면 쿼리문자열을 자동으로 만들어서 전달한다
		 */
		model.addAttribute("c_writer",commentVO.getC_writer());
